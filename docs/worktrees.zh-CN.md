...
worktree 切换器决定 Pi Task 接下来使用哪个 checkout。
...
如果这个 branch 已存在，Pi Task 会为它添加 worktree。如果 branch 不存在，Pi Task 会从当前 `HEAD` 创建这个 branch。
...
如果 worktree 里有未提交或未跟踪文件，Git 会拒绝删除。Pi Task 会再显示 force remove。force remove 会丢弃这个 checkout 里的未提交文件，只在确定不需要这些改动时使用。
...
Git 有时会保留 prunable worktree 记录。Pi Task 会过滤这些记录，不在切换器里显示。
