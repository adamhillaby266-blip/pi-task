...
需要升级源码时，先在原源码目录取得经审查的新版本。若 `package.json` 或锁文件发生变化，应先审查差异，再明确运行 `npm ci --include=dev --ignore-scripts`；该命令会下载依赖并重建 `node_modules`，不会迁移 Pi 对话或 Task 数据。升级器会在停服前执行依赖一致性检查，发现旧依赖时直接终止，不会静默用旧 SDK 构建。

依赖一致后，双击 Finder 中的 `bin/upgrade-pi-task-macos.command`，确认当前没有活动 Run 后输入 `UPGRADE`。它会：先进行隔离构建和页面冒烟，再短暂停止后台服务；停服后把 Task SQLite 一致性备份到 `~/.pi-task-backups/`，在独立副本上完成 schema 迁移与行数、完整性回读；随后才提升已验证构建并恢复原有 LaunchAgent。若备份、迁移验证或新构建启动失败，会停止升级并尝试恢复原服务或上一份 `.next`。全过程不修改 Pi 对话和认证，不会发送模型提示词；原 Task 数据只有在新版本实际启动并访问 Task 功能时才会执行已验证的增量迁移。
...
若输入框上方提示原工作目录已不存在，这是旧对话保存的目录被删除，不是登录失效。不要重新登录或新增 Provider：

1. 点击左上角当前工作目录；
2. 选择“自定义路径”；
3. 选择一个存在的工作目录，例如 `~/Projects/example-project`；
