...
import type { ModelLoadIssue } from "@/lib/model-load-state";
...
interface Props {
  ...
  modelLoadIssue?: ModelLoadIssue | null;
...
export function ModelLoadIssueBanner({ issue }: { issue?: ModelLoadIssue | null }) {
  const { t } = useI18n();
  if (!issue) return null;
  const body = issue === "cwd_unavailable"
    ? t("chat.modelCwdUnavailable")
    : issue === "access_denied"
      ? t("chat.modelAccessDenied")
      : t("chat.modelLoadUnavailable");
  return <ModelNoticeBanner tone="warning" title={t("chat.modelUnavailable")} body={body} />;
}
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  onSend, onAbort, onSteer, onFollowUp, isStreaming, model, isAutoModelSelection, modelNames, modelList, modelLoading, modelError, modelLoadIssue, modelScopeWarnings, onModelChange, onRefreshModels,
  ...
  const modelSelectorLabel = currentName
    ?? (modelLoading
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        <ModelLoadIssueBanner issue={modelLoadIssue} />
        ...
        <div style={{
          ...
          <div style={{ flex: isMobile ? "1 1 auto" : "0 0 auto", minWidth: 0, display: "flex", alignItems: "center", gap: 2 }}>
            ...
            {/* Always leave a model affordance visible. A missing project cwd or
                failed model load must explain itself instead of making selection vanish. */}
            {onModelChange && (
                <div ref={dropdownRef} style={{ position: "relative", flex: isMobile ? "1 1 auto" : undefined, minWidth: 0 }}>
                  <button
                    ...
                    title={modelOptions.length > 0 ? "Change model" : (modelLoading ? t("chat.loadingModels") : t("chat.noAvailableModels"))}
                  >
                      ...
                      <div style={{ minHeight: 0, overflowY: "auto" }}>
                        {modelsByProvider.length === 0 ? (
                          <div style={{ padding: "8px 12px", color: "var(--text-dim)", fontSize: 12 }}>
                            ...
                            {!modelFilter.trim() && onRefreshModels && (
                              <button
                                type="button"
