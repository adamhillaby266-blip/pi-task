...
export function AppShell() {
  ...
  useEffect(() => {
    ...
    if (pendingTaskStart) return;
    let cancelled = false;
    setResolvedTaskSessionId(null);
    void fetch(`/api/tasks?sessionId=${encodeURIComponent(sessionId)}`, { cache: "no-store" })
      .then((response) => {
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        return response.json() as Promise<{ task: TaskDetail | null }>;
      })
      .then((body) => {
        if (cancelled) return;
        setActiveTask(body.task);
      ...
      .catch(() => {
        if (!cancelled) {
          setActiveTask(null);
          setTaskActionError("暂时无法确认当前对话是否已绑定任务");
        }
      });
  ...
  const handleProcessTask = useCallback((
    ...
    const taskSession: SessionInfo = {
      ...
      created: timestamp,
      modified: timestamp,
      messageCount: task.runs.length,
      firstMessage: task.title,
      projectRoot: task.project.rootPath,
    };
    setSelectedSession(taskSession);
    setNewSessionCwd(null);
  ...
  const handleContinueTask = useCallback(async () => {
    ...
    if (!task || task.status !== "ready" || taskActionPending) return;
    ...
    try {
      ...
      handleProcessTask(task, body.session);
    } catch (cause) {
      setTaskActionError(cause instanceof Error ? cause.message : String(cause));
    } finally {
      setTaskActionPending(false);
    }
  }, [activeTask, handleProcessTask, taskActionPending]);
  ...
  const handleTaskReview = useCallback(async (action: "accept" | "return", reason?: string) => {
    ...
    if (!current) throw new Error("当前对话没有绑定任务");
    const response = await fetch(`/api/tasks/${encodeURIComponent(current.id)}/review`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        action,
    ...
    const prepareResponse = await fetch(`/api/tasks/${encodeURIComponent(body.task.id)}/prepare`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ version: body.task.version }),
    });
    const preparedBody = await prepareResponse.json().catch(() => ({})) as {
      session?: PreparedSession;
      error?: string | { message?: string };
    };
    if (!prepareResponse.ok || !preparedBody.session) {
      const message = typeof preparedBody.error === "string" ? preparedBody.error : preparedBody.error?.message;
      throw new Error(message || `HTTP ${prepareResponse.status}`);
    }
    handleProcessTask(body.task, preparedBody.session);
  }, [activeTask, handleProcessTask]);
  ...
  const refreshActiveTask = useCallback(async () => {
    ...
    try {
      ...
      const body = await response.json() as { task?: TaskDetail };
      const refreshedTask = body.task;
      if (refreshedTask) setActiveTask((current) => current?.id === taskId ? refreshedTask : current ?? null);
    } catch {
      // Task status refresh is opportunistic; the existing Task state remains readable.
    }
  }, [activeTask?.id]);
  ...
  const activeTaskRun = activeTask?.activeRunId
    ? activeTask.runs.find((run) => run.id === activeTask.activeRunId) ?? null
    : null;
  const activeTaskLabel = activeTaskRun?.status === "waiting_user"
    ? "等待你决定"
    : activeTask?.status === "ready" && activeTask.recoveryNote
      ? "待继续处理"
      : activeTask
        ? TASK_STATUS_LABELS[activeTask.status]
        : null;
  const hasPersistedConversationMessages = Boolean(
    selectedSession && (sessionStats?.userMessages ?? selectedSession.messageCount) > 0,
  );
  const canConvertConversation = Boolean(
    selectedSession
    && resolvedTaskSessionId === selectedSession.id
    && !activeTask
    && hasPersistedConversationMessages,
  );
  ...
  const windowTitle = mainView === "tasks"
    ? "任务 - Pi Task"
    : activeCwdName ? `${activeCwdName} - Pi Task` : "Pi Task";
    ...
    <div style={{
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        ...
        <div ref={topBarRef} style={{ display: "flex", alignItems: "center", flexShrink: 0, borderBottom: "1px solid var(--border)", height: "calc(36px + env(safe-area-inset-top))", paddingTop: "env(safe-area-inset-top)", background: "var(--bg-panel)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", height: "100%", borderRight: "1px solid var(--border)", padding: "0 4px", gap: 2 }}>
            <button
              type="button"
              onClick={() => setMainView("agent")}
              aria-pressed={mainView === "agent"}
              style={{
                display: "flex", alignItems: "center", gap: 5, height: 28, padding: isMobile ? "0 7px" : "0 9px",
                border: "none", borderRadius: 7,
                background: mainView === "agent" ? "var(--bg-selected)" : "none",
                color: mainView === "agent" ? "var(--text)" : "var(--text-muted)",
                font: "inherit", fontSize: 11, fontWeight: 650, cursor: "pointer",
              }}
            >
            ...
            <button
              type="button"
              onClick={() => { setMainView("tasks"); setActiveTopPanel(null); }}
              aria-pressed={mainView === "tasks"}
              style={{
                display: "flex", alignItems: "center", gap: 5, height: 28, padding: isMobile ? "0 7px" : "0 9px",
                border: "none", borderRadius: 7,
                background: mainView === "tasks" ? "var(--bg-selected)" : "none",
                color: mainView === "tasks" ? "var(--text)" : "var(--text-muted)",
                font: "inherit", fontSize: 11, fontWeight: 650, cursor: "pointer",
              }}
            >
        ...
        {mainView === "agent" && canConvertConversation && (
          <div style={{
            display: "flex", alignItems: "center", gap: 8, minHeight: 36, padding: "5px 10px",
            borderBottom: "1px solid var(--border)", background: "var(--bg-panel)", fontSize: 10,
          }}>
            <span style={{ width: 7, height: 7, borderRadius: "50%", border: "1.5px solid var(--text-dim)", flexShrink: 0 }} />
        ...
        {mainView === "agent" && activeTask && (
          <div style={{
            display: "flex", alignItems: "center", gap: 8, minHeight: 36, padding: "5px 10px",
            borderBottom: "1px solid var(--border)", background: "color-mix(in srgb, var(--accent) 5%, var(--bg-panel))",
            fontSize: 10,
          }}>
            <span style={{ width: 7, height: 7, borderRadius: "50%", background: activeTaskRun?.status === "waiting_user" ? "#7c3aed" : activeTask.status === "in_review" ? "#7c3aed" : activeTask.status === "in_progress" ? "#d97706" : activeTask.recoveryNote ? "#b45309" : "var(--accent)", flexShrink: 0 }} />
            <strong style={{ flex: "1 1 auto", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={activeTask.title}>{activeTask.title}</strong>
            <span style={{ color: activeTaskRun?.status === "waiting_user" ? "#7c3aed" : activeTask.recoveryNote ? "#b45309" : "var(--text-muted)", whiteSpace: "nowrap" }} title={activeTask.recoveryNote ?? undefined}>
              {activeTaskLabel}
            </span>
            {activeTask.status === "ready" && (
              <button
                ...
                style={{ padding: "4px 8px", border: "1px solid var(--accent)", borderRadius: 6, background: "var(--accent)", color: "white", font: "inherit", fontSize: 9, fontWeight: 650, cursor: taskActionPending ? "wait" : "pointer", opacity: taskActionPending ? .58 : 1, whiteSpace: "nowrap" }}
              >{taskActionPending ? "准备中…" : "继续处理"}</button>
            )}
            <button
              type="button"
              onClick={() => setMainView("tasks")}
              style={{ padding: "4px 8px", border: "1px solid var(--border)", borderRadius: 6, background: "var(--bg-panel)", color: "var(--text-muted)", font: "inherit", fontSize: 9, cursor: "pointer", whiteSpace: "nowrap" }}
            >查看任务</button>
          </div>
        )}
        {mainView === "agent" && taskActionError && (
          <div role="alert" style={{ display: "flex", alignItems: "center", gap: 8, minHeight: 30, padding: "4px 10px", borderBottom: "1px solid color-mix(in srgb, #dc2626 32%, var(--border))", background: "color-mix(in srgb, #dc2626 7%, var(--bg-panel))", color: "#dc2626", fontSize: 9 }}>
            <span style={{ flex: 1 }}>{taskActionError}</span>
            <button type="button" onClick={() => setTaskActionError(null)} style={{ border: 0, background: "none", color: "inherit", cursor: "pointer", font: "inherit" }}>关闭</button>
          </div>
        )}
