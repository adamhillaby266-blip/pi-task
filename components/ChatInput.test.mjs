...
test("keeps a model affordance visible when an empty list has no runtime error", () => {
  const html = renderToStaticMarkup(
    React.createElement(
      I18nProvider,
      null,
...
test("explains when a restored session points to a missing project folder", () => {
  const html = renderToStaticMarkup(
    React.createElement(
      I18nProvider,
      null,
      React.createElement(ModelLoadIssueBanner, { issue: "cwd_unavailable" }),
    ),
  );

  assert.match(html, /Model selection unavailable/);
  assert.match(html, /project folder is no longer available/);
});
