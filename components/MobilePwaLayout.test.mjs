...
test("switches narrow tablets to the mobile shell before the desktop UI becomes cramped", () => {
  assert.match(mobileHookSource, /MOBILE_QUERY = "\(max-width: 760px\)"/);
  assert.match(cssSource, /\/\* Mobile and narrow-tablet sidebar:[\s\S]*?@media \(max-width: 760px\)/);
  assert.match(appShellSource, /@media \(max-width: 760px\)/);
});
