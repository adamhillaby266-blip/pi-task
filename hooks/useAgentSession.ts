...
import { resolveModelLoadIssue, type ModelLoadIssue } from "@/lib/model-load-state";
...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const [modelLoadIssue, setModelLoadIssue] = useState<ModelLoadIssue | null>(null);
  ...
  const handleAgentEvent = useCallback((event: AgentEvent) => {
    switch (event.type) {
      ...
      case "prompt_error": {
        ...
        void interruptActiveTaskRun(message);
        break;
      }
  ...
  }, [addNotice, cancelEventStreamGrace, handleExtensionUiRequest, interruptActiveTaskRun, loadSession, notifyPromptStage, onAgentEnd, scheduleEventStreamClose, settleUiStage]);
  ...
  const startPendingTask = useCallback(async (sessionId: string): Promise<RunRecord | null> => {
    ...
    if (!response.ok || !body.task || !body.run) {
    ...
    try {
      if (isNew && newSessionCwd) {
        ...
        if (sid) {
          ...
          taskRunStarted = Boolean(await startPendingTask(sid));
        ...
        taskRunStarted = Boolean(await startPendingTask(session.id));
  ...
  const loadModels = useCallback(async (signal?: AbortSignal, forceRefresh = false) => {
    ...
    const modelsUrl = params.size > 0 ? `/api/models?${params.toString()}` : "/api/models";
    ...
    try {
      ...
      const d = await res.json().catch(() => ({})) as ModelsResponse;
      if (signal?.aborted || requestId !== modelLoadRequestRef.current) return;
      if (!res.ok) {
        setModelNames({});
        setModelList([]);
        setModelError(null);
        ...
        if (isNew && !sessionIdRef.current) setNewSessionDefaultModel(null);
        setModelLoadIssue(resolveModelLoadIssue(res.status, d.errorCode));
        return;
      }
      ...
      setModelThinkingLevelMaps({});
      if (isNew && !sessionIdRef.current) setNewSessionDefaultModel(null);
      setModelLoadIssue("unavailable");
    } finally {
      if (!signal?.aborted && requestId === modelLoadRequestRef.current) setModelsLoading(false);
    }
  }, [isNew, newSessionCwd, session?.cwd]);

  const refreshModels = useCallback(() => loadModels(undefined, true), [loadModels]);
  ...
  return {
    ...
    agentRunning, modelNames, modelList, modelsLoading, modelError, modelLoadIssue, modelScopeWarnings, modelThinkingLevels, modelThinkingLevelMaps, newSessionModel, toolPreset, thinkingLevel,
    ...
    handleToolPresetChange, handleThinkingLevelChange, refreshModels, loadTools, loadSlashCommands, setActiveLeafId, setData, setMessages,
