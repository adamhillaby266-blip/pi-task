...
test("rejects LAN hosts even when browser headers report a matching origin", async () => {
  const { isApiRequestAllowed } = await loadSubject();
...
test("allows IPv6 loopback and rejects a configured external hostname", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  ...
  const external = new Request("http://localhost:30141/api/test", {
    method: "POST",
    headers: {
  ...
  assert.equal(isApiRequestAllowed(external), false);
});
...
test("rejects an origin that does not match the loopback request host", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  const request = new Request("http://localhost:30141/api/test", {
    method: "POST",
    headers: {
