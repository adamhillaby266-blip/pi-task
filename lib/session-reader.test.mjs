...
function framingContract(title, blocked = false) {
  const item = (id, text, status = "confirmed") => ({ id, text, status });
  return {
    schemaVersion: 1,
    title,
    outcome: item("outcome", "形成可复核的虚构分析"),
    audience: [item("audience", "供虚构负责人判断")],
    authoritativeSources: [{ ...item("source", "虚构权威表"), availability: "available" }],
    scope: { included: [], excluded: [] },
    deliverables: [{ ...item("deliverable", "分析结果"), kind: "file" }],
    acceptanceCriteria: [item("acceptance", "结果可回读")],
    constraints: [],
    assumptions: [],
    openDecisions: blocked ? [{ id: "decision", question: "采用哪个口径？", blocking: true, status: "open" }] : [],
    gates: [],
  };
}

function framingEntry(id, parentId, revision, title, blocked = false) {
  return {
    type: "custom",
    id,
    parentId,
    timestamp: `2026-01-01T00:00:0${revision}.000Z`,
    customType: "pi-task.task-framing",
    data: {
      schemaVersion: 1,
      eventType: "draft",
      draftId: "tfd_session_reader",
      revision,
      replacesEntryId: revision > 1 ? `draft${revision - 1}` : null,
      taskId: null,
      baseTaskVersion: null,
      contract: framingContract(title, blocked),
      changeSummary: revision > 1 ? ["更新口径"] : [],
      createdBy: "agent",
    },
  };
}
...
test("projects Task Framing custom state as UI-only messages with supersession metadata", () => {
  const entries = [
    userEntry("u1", null, "start framing"),
    framingEntry("draft1", "u1", 1, "草案一", true),
    userEntry("u2", "draft1", "resolve decision"),
    framingEntry("draft2", "u2", 2, "草案二"),
  ];

  const context = buildSessionContext(entries, "draft2");
  const drafts = context.messages.filter((message) => message.role === "custom" && message.customType === "pi-task.task-framing");

  assert.deepEqual(context.entryIds, ["u1", "draft1", "u2", "draft2"]);
  assert.equal(drafts.length, 2);
  assert.equal(drafts[0].details.isLatestDraft, false);
  assert.equal(drafts[0].details.supersededByEntryId, "draft2");
  assert.equal(drafts[1].details.isLatestDraft, true);
  assert.equal(drafts[1].details.readiness.ready, true);
});

test("restores the latest Task Framing draft after compaction without restoring every old draft", () => {
  const entries = [
    userEntry("u1", null, "old request"),
    framingEntry("draft1", "u1", 1, "压缩前草案"),
    userEntry("u2", "draft1", "kept request"),
    {
      type: "compaction",
      id: "cmp",
      parentId: "u2",
  ...
  const context = buildSessionContext(entries, "u3");
  const restored = context.messages.at(-1);

  assert.deepEqual(context.entryIds, ["cmp", "u2", "u3", "draft1"]);
  assert.equal(restored.role, "custom");
  assert.equal(restored.customType, "pi-task.task-framing");
  assert.equal(restored.details.restoredAfterCompaction, true);
  assert.equal(restored.details.event.contract.title, "压缩前草案");
});

test("Task Framing projection follows the selected Session branch", () => {
  const entries = [
    userEntry("root", null, "root"),
    framingEntry("draft1", "root", 1, "共享草案"),
    userEntry("main", "draft1", "main"),
    framingEntry("draft2", "main", 2, "主分支草案"),
    userEntry("alternate", "draft1", "alternate"),
  ];

  const context = buildSessionContext(entries, "alternate");
  const drafts = context.messages.filter((message) => message.role === "custom" && message.customType === "pi-task.task-framing");

  assert.deepEqual(context.entryIds, ["root", "draft1", "alternate"]);
  assert.equal(drafts.length, 1);
  assert.equal(drafts[0].details.event.contract.title, "共享草案");
});
