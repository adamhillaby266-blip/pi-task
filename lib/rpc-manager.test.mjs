...
test("RPC sessions always preload the trusted Task Framing Extension before optional extensions", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  assert.match(source, /import \{ createTaskFramingExtension \} from "\.\/task\/framing-extension"/);
  assert.match(startupSource, /const trustedExtensionFactories = \[createTaskFramingExtension\(\), \.\.\.\(extensionFactories \?\? \[\]\)\]/);
  assert.match(startupSource, /resourceLoaderOptions: \{ extensionFactories: trustedExtensionFactories \}/);
});
...
test("a new top-level prompt refreshes stale network sockets only while all sessions are idle", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  ...
  assert.match(source, /import \{ refreshHttpDispatcherForNewPrompt \} from "\.\/http-dispatcher"/);
  assert.match(promptSource, /if \(getRunningRpcSessionIds\(\)\.length === 0\) \{\s*refreshHttpDispatcherForNewPrompt\(\);\s*\}/);
  assert.ok(
    promptSource.indexOf("refreshHttpDispatcherForNewPrompt()") < promptSource.indexOf("this.promptRunning = true"),
    "network sockets must refresh before the model request begins",
  );
});
