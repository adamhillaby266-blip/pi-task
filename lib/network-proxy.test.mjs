...
test("standard environment variables take precedence over local and macOS settings", () => {
  ...
  let macosReads = 0;
  const resolution = resolveNetworkProxyConfiguration({
    environment,
    platform: "darwin",
  ...
  assert.equal(resolution.status.http.source, "environment");
  assert.equal(resolution.status.https.source, "environment");
  assert.equal(resolution.status.noProxy.source, "environment");
  assert.deepEqual(resolution.agentOptions, {
    httpProxy: "http://environment.example.test:8080",
    httpsProxy: "http://environment.example.test:8080",
...
test("uses macOS system HTTP(S) proxy only when no explicit configuration exists", () => {
  ...
  assert.deepEqual(resolution.status.http, { source: "macos-system", enabled: true });
  assert.deepEqual(resolution.status.https, { source: "macos-system", enabled: true, fallsBackToHttp: false });
  assert.deepEqual(resolution.status.noProxy, { source: "default", enabled: true });
  assert.equal(JSON.stringify(resolution.status).includes("127.0.0.1:7890"), false);
});
...
test("uses a private local config before a macOS system proxy", () => {
  ...
  assert.equal(environment.HTTPS_PROXY, undefined);
  assert.equal(resolution.agentOptions.httpsProxy, "http://127.0.0.1:7890");
  assert.equal(resolution.status.https.source, "local-config");
  assert.equal(resolution.status.http.source, "direct");
  assert.equal(resolution.status.localConfig, "loaded");
});
...
test("does not inspect macOS proxy settings when local config defines both protocols", () => {
  ...
  assert.equal(macosReads, 0);
  assert.equal(resolution.status.macosSystem, "unchecked");
  assert.equal(resolution.status.http.source, "local-config");
  assert.equal(resolution.status.https.source, "local-config");
});
...
test("reports unsupported macOS SOCKS/PAC-only settings without leaking endpoint details", () => {
  ...
  assert.equal(resolution.status.https.enabled, false);
  assert.deepEqual(resolution.status.warnings, ["macos_socks_proxy_requires_http", "macos_pac_proxy_requires_http"]);
  assert.equal(JSON.stringify(getNetworkProxyStatus()).includes("127.0.0.1"), false);
});

test("re-reads a changed macOS system proxy without making it an environment override", () => {
  const environment = {};
  let activePort = 7890;
  const resolve = () => resolveNetworkProxyConfiguration({
    environment,
    platform: "darwin",
    homeDirectory: "/Users/test",
    readConfig: () => ({ values: {}, state: "missing", warnings: [] }),
    readMacosProxy: () => macosResult({
      ...EMPTY_MACOS_PROXY,
      httpProxy: `http://127.0.0.1:${activePort}`,
      httpsProxy: `http://127.0.0.1:${activePort}`,
    }),
  });

  const beforeSwitch = resolve();
  activePort = 7891;
  const afterSwitch = resolve();

  assert.equal(environment.HTTP_PROXY, undefined);
  assert.equal(environment.HTTPS_PROXY, undefined);
  assert.equal(beforeSwitch.agentOptions.httpsProxy, "http://127.0.0.1:7890");
  assert.equal(afterSwitch.agentOptions.httpsProxy, "http://127.0.0.1:7891");
  assert.notEqual(beforeSwitch.fingerprint, afterSwitch.fingerprint);
});
