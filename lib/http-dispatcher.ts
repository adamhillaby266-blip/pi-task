...
import { configureNetworkProxyEnvironment, describeNetworkProxyStatus } from "./network-proxy";
...
export function configureHttpDispatcher(
  ...
  const networkProxyStatus = configureNetworkProxyEnvironment();
  if (networkProxyStatus.warnings.length > 0) {
    console.warn(`[pi-task] network proxy configuration warnings: ${networkProxyStatus.warnings.join(", ")}`);
  }
  if (networkProxyStatus.http.enabled || networkProxyStatus.https.enabled) {
    console.log(`[pi-task] ${describeNetworkProxyStatus(networkProxyStatus)}`);
  }
