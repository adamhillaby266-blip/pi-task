# Pi Task - Development Notes

## Product rules

- Product name: **Pi Task**; repository name: `pi-task`.
- Pi Agent conversation is the UX center. Tasks connect goals, runs, artifacts, reviews, and recovery; do not create a separate Run workspace that replaces chat.
- Gate C’s manual two-Run lifecycle and browser evidence have passed. Gate D focuses on conversation-to-task creation and same-Session recovery; automation, scheduling, multi-Agent execution, analytics, workflow editing, and production packaging remain out of scope.
- Task and Run are separate state machines. Never infer business completion from Pi streaming or the final assistant message.
- Agent capabilities cannot accept Reviews or mark Tasks done. User and Agent mutations use separate server-side paths.
- Task data belongs in the Pi Task SQLite store, not Pi Session JSONL. Associate them by `sessionId`.
- Keep upstream provenance in `UPSTREAM.md` and the MIT notice in `LICENSE`.
- The imported Pi Web baseline is commit `99b1b00`; do not overwrite product changes by recopying upstream files.
- During isolated validation, set `HOME`, `TMPDIR`, `PI_CODING_AGENT_DIR`, and `PI_TASK_DATA_DIR` to directories under `.runtime/`. Never use company or real unpublished data in fixtures.
