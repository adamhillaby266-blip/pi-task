...
function modelErrorResponse(errorCode: "cwd_unavailable" | "cwd_not_directory" | "access_denied" | "unavailable", status: number) {
...
export async function GET(req: Request) {
  ...
  try {
    ...
    return modelErrorResponse("cwd_unavailable", 400);
  }
  if (!cwdStat.isDirectory()) {
    return modelErrorResponse("cwd_not_directory", 400);
  }
  const allowedRoots = await getAllowedFileRoots();
  if (!isExistingFilePathAllowed(cwd, allowedRoots)) {
    return modelErrorResponse("access_denied", 403);
  }

  // A user-triggered retry must not wait for the short per-cwd cache TTL. It
  // only rebuilds Pi's local runtime snapshot; it does not invoke a model.
  if (url.searchParams.get("refresh") === "1") invalidateModelsCache();

  try {
    return Response.json(await loadModelsWithCache(cwd, () => loadModels(cwd)));
  } catch {
    return modelErrorResponse("unavailable", 500);
  }
}
